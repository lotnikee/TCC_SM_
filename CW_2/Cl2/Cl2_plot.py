...
data = pd.ExcelFile('~/Desktop/TCC_SM_/CW_2/Cl2/SM_CW2_Cl2.xlsx')
Cl2_data_sheet = data.parse('Sheet1') 
temperature_column = Cl2_data_sheet.columns[0]
cv_nist_column = Cl2_data_sheet.columns[2]

filtered_Cl2_data = Cl2_data_sheet[[temperature_column, cv_nist_column]].iloc[2:61]
filtered_Cl2_data.columns = ['T', 'C_v (NIST)']
...
plt.plot(filtered_Cl2_data['T'], filtered_Cl2_data['C_v (NIST)'], label='Data (NIST Webbook; Chase, 1998)', color='red')
# Plot design 
plt.xlabel('Temperature (K)', fontsize=14, weight='bold')
plt.ylabel('$C_v$ (eV/K) * E-04', fontsize=14, weight='bold')
plt.title('Constant Volume Heat Capacity $C_v$ of $Cl_2$', fontsize=16, weight='bold')
